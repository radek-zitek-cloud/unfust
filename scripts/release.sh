...
log_info "Step 4: Creating release branch..."
...
python3 "${SCRIPT_DIR}/changelog.py" --version "${TAG}" --output "${CHANGELOG_FILE}"
...
if [ "${DRY_RUN}" == "true" ]; then
    ...
    echo "  2. Deploy using: docker pull ${BACKEND_IMAGE}:${NEW_VERSION}"
fi
